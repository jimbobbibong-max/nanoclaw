...
import { seedScheduledTasks } from './seed-tasks.js';
...
async function main(): Promise<void> {
  ...
  initDatabase();
  seedScheduledTasks();
  const activeTasks = getAllTasks().filter((t) => t.status === 'active');
  logger.info(
    { total: activeTasks.length, taskIds: activeTasks.map((t) => t.id) },
    'Database initialized — scheduled tasks loaded',
  );
  loadState();
